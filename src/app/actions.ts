...
const common=z.object({date:dateSchema,painScore:z.union([z.literal(''),z.coerce.number().int().min(0).max(10)]),area:z.string().trim().min(1).max(100),treatmentAreas:z.array(z.string().trim().min(1).max(100)).min(1).max(10),caseDescription:z.string().trim().max(300),romNotes:z.string().trim().max(2000),subjective:z.string().trim().max(4000),objective:z.string().trim().max(4000),assessment:z.string().trim().max(4000),plan:z.string().trim().max(4000),delta:z.string().trim().max(4000)});
function englishDraft(input:z.infer<typeof common>,identifier:string,kind:string){
 const sections=[['Visit',`${input.date} · ${kind}`],['Patient identifier',identifier],['Case description',input.caseDescription],['Treatment areas',input.treatmentAreas.join('; ')],['Range of motion',input.romNotes],['Patient report',input.subjective],['Objective findings',input.objective],['Assessment',input.assessment],['Plan',input.plan],['Change since prior visit',input.delta],['Pain score',input.painScore===''?'Not recorded':`${input.painScore}/10`]];
 return sections.filter(([,value])=>value).map(([title,value])=>`${title}: ${value}`).join('\n\n');
}
export async function createPatient(form:FormData){
 await requireAccess();
 const identifier=text(form.get('identifier'),80).toUpperCase();
 const treatmentAreas=text(form.get('treatmentAreas'),1200).split(/\r?\n/).map(v=>v.trim()).filter(Boolean).slice(0,10);
 const parsed=common.safeParse({date:form.get('date'),painScore:form.get('painScore')??'',area:treatmentAreas[0]??'',treatmentAreas,caseDescription:text(form.get('caseDescription'),300),romNotes:text(form.get('romNotes'),2000),subjective:text(form.get('subjective')),objective:text(form.get('objective')),assessment:text(form.get('assessment')),plan:text(form.get('plan')),delta:''});
 const p=payer.safeParse(form.get('payer'));
 if(!/^[A-Z0-9][A-Z0-9-]{2,79}$/.test(identifier)||!p.success||!parsed.success)throw new Error('Please check the patient ID, payer, date, treatment areas, and pain score (0–10).');
 ...
 await db.$transaction(async tx=>{await tx.patient.create({data:{id:patientId,identifier,payer:p.data,primaryTreatmentArea:parsed.data.treatmentAreas[0]!,caseDescription:parsed.data.caseDescription||null,caseOpenedDate:visitDate,isDemo:true}});await tx.encounter.create({data:{id:encounterId,patientId}});await tx.visit.create({data:{id:visitId,patientId,encounterId,date:visitDate,kind:'initial',painScore:parsed.data.painScore===''?null:parsed.data.painScore as number,romData:parsed.data.romNotes?{notes:parsed.data.romNotes}:{},treatmentAreas:parsed.data.treatmentAreas,narrativeDelta:parsed.data.subjective,generatedNote:note,payerFlags:[{type:'source_status',message:'No payer rules have been verified from source documents.',resolved:false}]}});});
...
export async function saveFollowUp(form:FormData){
 ...
 const area=text(form.get('area'),100);const parsed=common.safeParse({date:form.get('date'),painScore:form.get('painScore')??'',area,treatmentAreas:[area],caseDescription:'',romNotes:text(form.get('romNotes'),2000),subjective:text(form.get('subjective')),objective:text(form.get('objective')),assessment:text(form.get('assessment')),plan:text(form.get('plan')),delta:text(form.get('delta'))});
 ...
 await db.$transaction(async tx=>{if(!target)await tx.encounter.create({data:{id:encounterId,patientId}});const current=target?await tx.visit.findMany({where:{encounterId},orderBy:{version:'desc'},take:1}):[];const version=(current[0]?.version??0)+1;await tx.visit.create({data:{id:randomUUID(),patientId,encounterId,version,date,kind:target?.kind??'follow_up',painScore:parsed.data.painScore===''?target?.painScore??null:parsed.data.painScore as number,romData:target?.romData??(parsed.data.romNotes?{notes:parsed.data.romNotes}:{}),treatmentAreas:target?.treatmentAreas??parsed.data.treatmentAreas,narrativeDelta:parsed.data.delta||parsed.data.subjective||target?.narrativeDelta||'',generatedNote:note,payerFlags:[{type:'source_status',message:'No payer rules have been verified from source documents.',resolved:false}]}});});
